...
- 再起動フロー
  - ハンズオン:
    ...
    - ログ確認より正常に起動しているか確認
- ハンズオン:
  - アプリコード編集により自動でコンテナを再ビルド(nodemon)
  - log リアルタイムで表示 `docker compose logs -f app`
  - log 直近〇件表示 `docker compose logs --tails=100 app`
  - log 直近〇分表示 `docker compose logs --since=10m app`
...
- ハンズオン:
  - `docker compose up -d` redisを起動
  - `docker compose --profile normal up -d` 正常系
  - `docker compose --profile fail-startup up -d` コマンド失敗系
    - package.jsonのscriptsを見直し
  - `docker compose --profile fail-connectivity up -d` 接続失敗系
    - environmentのREDIS_HOSTを見直し
  - `docker compose --profile fail-permission up -d`後にhttp://localhost/write へアクセス 権限無し
    - 権限の見直しもしくは対象となるファイルへ権限を付与
  - 検証コマンド
    - `docker logs` log確認
    - `docker inspect` コンテナ環境情報の詳細を確認
    - `docker compose exec` コンテナ内でコマンド検証
...
# imageビルド
docker build ./ -t docker-study-app:local
# コンテナ起動(停止時に自動削除オプション)
docker run --rm -p 3000:3000 docker-study-app:local
# composeファイルからbuild ~ run までを実行(イメージ再ビルド)
